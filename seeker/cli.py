...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    if args.play_gemini_audio:
        config.gemini.play_audio_response = args.play_gemini_audio
    if args.speed != 1.0:
        config.audio.play_speed = args.speed
    if args.mode:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp_start.add_argument("--play-gemini-audio", action="store_true", help="Play the audio response generated by Gemini")
    sp_start.add_argument("--speed", type=float, default=1.0, help="Playback speed multiplier for ingested audio (e.g. 1.5)")
